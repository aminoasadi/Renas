...
export const handleImageUpload = async (
  ...
  // The upload API doesn't report incremental progress, so this just marks
  // the two states the toolbar's progress UI actually distinguishes:
  // "started" and "done" (or thrown on failure/abort).
  onProgress?.({ progress: 0 })
  if (abortSignal?.aborted) {
    throw new Error("Upload cancelled")
  }
  const asset = await api<{ publicUrl: string }>("/media/upload", {
    method: "POST",
    body: formData,
    isFormData: true,
  })
